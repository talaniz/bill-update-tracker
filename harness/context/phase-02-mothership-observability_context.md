...
- ntfy's canonical base URL must remain host-and-port only (`http://deathstar.local:8093`) and its authenticated service port is LAN-reachable; nginx continues to expose the LAN convenience route at `/ntfy/`, which is not a valid ntfy base URL.
